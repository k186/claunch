...
# ca --resume     — pick a session to resume, restoring its recorded model
...
if [[ "$_CA_LANG" == "zh" ]]; then
  ...
  _S_REMOVE_PROMPT="删除 > "
  _S_RESUME_HDR=$'选择要恢复的会话    Del = 删除\n'
  _S_RESUME_PROMPT="恢复 > "
  _S_LAUNCHING="  正在启动: %s\n"
  _S_SWITCHED="  已切换至: %s\n"
  _S_CANCELLED="  已取消。"
  _S_CONFIRM_DEL='  删除模型 "%s"？此操作不可撤销。[Y/n]: '
  _S_SES_DEL1='  删除会话 "%s"？此操作不可撤销。[Y/n]: '
  _S_DELETED="  已删除: %s\n"
  _S_EDIT_TITLE="  编辑: %s\n"
  ...
  _S_RESUME_HDR=$'Select a session to resume    Del = delete\n'
  _S_RESUME_PROMPT="Resume > "
  ...
  _S_SES_DEL1='  Delete session "%s"? This cannot be undone. [Y/n]: '
...
launch_claude() {
  ...
  # `claude --resume <id>` only finds a session in the current project dir, so
  # when resuming we run claude from the session's project. A subshell cd keeps
  # the caller's cwd untouched. _CA_RUN_CWD records where claude actually ran,
  # for re-saving the session record with the right cwd.
  local run_dir="$PWD"
  if [[ -n "$_CA_RESUME_CWD" && -d "$_CA_RESUME_CWD" ]]; then
    run_dir="$_CA_RESUME_CWD"
  fi
  _CA_RUN_CWD="$run_dir"
  ...
  local _ca_rc
  if (( ${#cmd_env[@]} > 0 )); then
    ( cd "$run_dir" && env "${cmd_env[@]}" claude "${claude_args[@]}" )
  else
    ( cd "$run_dir" && claude "${claude_args[@]}" )
  fi
  _ca_rc=$?

  # Restore terminal state FIRST. A Ctrl-C exit can leave the alternate
  # screen active, so anything printed before we exit it (like the session
  # line below) would be discarded when the screen is restored.
...
_ca_apply_model_by_string() {
  ...
  # Exact pass: entry name, --model id, or env.ANTHROPIC_MODEL.
  for ((i=0; i<count; i++)); do
  ...
  # Loose pass: transcripts record ids without the [1m] context-window
  # suffix, so also compare normalized forms (suffix + whitespace stripped).
  # This lets e.g. a recorded "deepseek-v4-flash" match env.ANTHROPIC_MODEL
  # "deepseek-v4-flash[1m]" and restore the full provider env. Runs only
  # after the exact pass so "claude-opus-4-7[1m]" still wins its exact entry.
  local tnorm="${target//\[1m\]/}"
  tnorm="${tnorm//[[:space:]]/}"
  [[ -n "$tnorm" ]] || return 1
  local nn nm ne
  for ((i=0; i<count; i++)); do
    ...
    nm="${m//\[1m\]/}"; nm="${nm//[[:space:]]/}"
    ne="${e//\[1m\]/}"; ne="${ne//[[:space:]]/}"
    if [[ "$nn" == "$tnorm" || "$nm" == "$tnorm" || "$ne" == "$tnorm" ]]; then
      build_model_env "$i"
      _CA_MODEL_NAME="$n"
      return 0
    fi
...
#   ~/.config/claunch/sessions/<session-id>.json  →  {"model":"<model string>","cwd":"<project dir>"}
_ca_save_session() {
  local sid="$1" name="$2" cwd="$3" tmp
  [[ -n "$sid" && -n "$name" ]] || return 1
  mkdir -p "$SESSION_DIR"
  tmp=$(mktemp)
  if jq -n --arg m "$name" --arg c "${cwd:-}" '{model:$m, cwd:$c}' > "$tmp" 2>/dev/null \
...
_ca_session_cwd() {
  local sid="$1" f="$SESSION_DIR/$sid.json"
  [[ -f "$f" ]] || return 1
...
# Reverse of the "${PWD//[^A-Za-z0-9]/-}" project-dir encoding: given a
# transcript path under ~/.claude/projects/<encoded>, walk the encoded path
# from / and at each level descend into the real child whose encoding extends
# the target's encoded prefix. Lossy (both "_" and "-" encode to "-"), so this
# is best-effort; the caller falls back to the current dir when no
# unambiguous match is found.
_ca_decode_project_dir() {
  local dir="$1" target acc="/" d ep enc_acc found
  [[ -n "$dir" ]] || return 1
  target="${dir##*/}"                  # last segment = encoded cwd
  [[ "$target" == -* ]] || return 1
  while :; do
    enc_acc="${acc//[^A-Za-z0-9]/-}"
    if [[ "$target" == "$enc_acc" ]]; then
      [[ "$acc" == "/" ]] && return 1 || { echo "$acc"; return 0; }
    fi
    found=""
    for d in "${acc%/}/"*; do
      [[ -d "$d" ]] || continue
...
_ca_session_file_model() {
  local f="$1" m
  [[ -f "$f" ]] || return 1
  # Last real assistant model; skip "<synthetic>" summary markers, which carry
  # no usable model id and would fail the entry lookup when resuming.
  m=$(jq -r 'select(.type == "assistant" and (.message.model // "") != "") | .message.model' "$f" 2>/dev/null \
      | grep -v '^<synthetic>$' | tail -1)
  if [[ -z "$m" ]]; then
...
# Short readable nickname for a session: claude's own name for the
# conversation (transcript {"type":"ai-title","aiTitle":...} / agent-name).
# Returns "" when there's no nickname — callers then fall back to the id.
_ca_session_alias() {
  local f="$1" s
  [[ -f "$f" ]] || return 1
  s=$(jq -r 'select(.type=="ai-title") | .aiTitle // empty' "$f" 2>/dev/null | tail -1)
  [[ -z "$s" ]] && s=$(jq -r 'select(.type=="agent-name") | .agentName // empty' "$f" 2>/dev/null | tail -1)
  # Auto-titles that start with a "-" (e.g. "--continue 未恢复上次环境") are
  # flag/command artifacts, not usable nicknames — treat as "no name" so
...
# Also resolves the session's project dir into _CA_RESUME_CWD so launch_claude
# runs claude there — `claude --resume <id>` only finds a transcript inside the
# current project dir.
_ca_resume_session() {
  local sid="$1" m f rc=0
  [[ -z "$sid" ]] && return 1
  f=$(_ca_find_session_file "$sid")
  m=$(_ca_session_model "$sid") || m=""
  if [[ -n "$m" ]] && _ca_apply_model_by_string "$m"; then
    rc=0
  else
    [[ -n "$f" ]] || return 1
    m=$(_ca_session_file_model "$f") || m=""
    _ca_apply_model_by_string "$m"
    rc=$?
  fi

  _CA_RESUME_CWD=$(_ca_session_cwd "$sid")
  if [[ -z "$_CA_RESUME_CWD" && -n "$f" ]]; then
    _CA_RESUME_CWD=$(_ca_decode_project_dir "${f%/*}")
  fi
  return $rc
}
...
_ca_record_session() {
  ...
  [[ -d "$proj" ]] || return 1
  # Scope by the directory claude actually ran in (resume cd's to the
  # session's project), so a concurrent session elsewhere is never mistaken
  # for the one we just exited.
  local run_dir="${_CA_RUN_CWD:-$PWD}"
  local scope="$proj/${run_dir//[^A-Za-z0-9]/-}"
  [[ -d "$scope" ]] || return 1
  ...
  local m
  # Prefer the entry name we launched with — it maps unambiguously back to
  # the full provider env on resume. Fall back to the transcript model only
  # for plain pass-through launches (where no entry was selected).
  if [[ -n "$name" ]]; then
    m="$name"
  else
    m=$(_ca_session_file_model "$best") || m=""
  fi
  [[ -n "$m" ]] || return 1
  _ca_save_session "$sid" "$m" "$_CA_RUN_CWD"

  local tmp
  tmp=$(mktemp)
  if jq --arg id "$sid" '.last_session = $id' "$MODELS_CFG" > "$tmp" 2>/dev/null \
     && mv "$tmp" "$MODELS_CFG"; then
    # Surface the session (its readable alias, or the id) so it's easy to
    # resume later.
    # Exit line shows the resumable id directly; nicknames (auto-derived
    # ai-titles can be noise) are for the resume list, not here.
    printf "  Session: %s  (model: %s)\n" "$sid" "$m"
    return 0
  fi
  rm -f "$tmp"
  return 1
...
_ca_cmd_help() {
  if [[ "$_CA_LANG" == "zh" ]]; then
    ...
    echo "    ca --resume             选择会话后恢复"
    echo "    ca --resume <id>        恢复指定会话"
    echo ""
    echo "  模型管理:"
    echo ""
    ...
    echo "    ca --resume             Pick a session to resume"
    echo "    ca --resume <id>        Resume a specific session"
    echo ""
...
# Pick a session to resume from this project's transcripts under
# ~/.claude/projects/<encoded-cwd>/ (instead of claude's own raw picker).
# Shows a readable alias (derived from the first real prompt), falling back to
# the id; Del deletes a session after two confirmations.
# Prints the chosen session id.
_ca_cmd_resume() {
  local proj="$(_ca_claude_dir)/projects"
  local scope="$proj/${PWD//[^A-Za-z0-9]/-}"
  [[ -d "$scope" ]] || { echo "  $_S_CANCELLED"; return 1; }
  local -a files=()
  local sorted f sid alias m out key choice
  _ca_pick_id=""

  while :; do
    files=("$scope"/*.jsonl(N))
    (( ${#files[@]} > 0 )) || { echo "  $_S_CANCELLED"; return 1; }

    sorted=""
    for f in "${(@f)$(ls -t -- "${files[@]}" 2>/dev/null)}"; do
      sid=$(basename "$f" .jsonl)
      [[ "$sid" =~ ^[A-Za-z0-9_-]{1,128}$ ]] || continue
      alias=$(_ca_session_alias "$f")
      m=$(_ca_session_model "$sid") || m=$(_ca_session_file_model "$f")
      # Tab-separated: display nickname/id + model via --with-nth, keep the
      # id as a hidden last field for the selection.
      sorted+="${alias:-$sid}\t${m:-?}\t${sid}\n"
    done
    [[ -n "$sorted" ]] || { echo "  $_S_CANCELLED"; return 1; }

    out=$(printf '%b' "$sorted" | fzf \
      --height=~60% \
      --border=rounded \
      --margin=1,2 \
      --padding=1 \
      --reverse \
      --prompt="$_S_RESUME_PROMPT" \
      --header="$_S_RESUME_HDR" \
      --expect='del' \
      --delimiter=$'\t' \
      --with-nth=1,2 \
      --color=border:7 \
      --no-info 2>&1)
    [[ -n "$out" ]] || { echo "  $_S_CANCELLED"; return 1; }
    key=$(printf '%s\n' "$out" | head -1)
    choice=$(printf '%s\n' "$out" | tail -1)
    [[ -n "$choice" ]] || { echo "  $_S_CANCELLED"; return 1; }
    sid=$(printf '%s\n' "$choice" | awk -F '\t' '{print $3}')

    if [[ "$key" == "del" ]]; then
      echo ""
      printf "$_S_SES_DEL1" "$sid"
      read -r _c1
      if [[ "$_c1" != "y" && "$_c1" != "Y" ]]; then echo "  $_S_CANCELLED"; return 0; fi
      rm -f -- "$scope/$sid.jsonl"
      rm -f -- "$SESSION_DIR/$sid.json"
      echo ""
      printf "$_S_DELETED" "$sid"
      echo ""
      continue
    fi
    _ca_pick_id=$(printf '%s\n' "$choice" | awk '{print $NF}')
    return 0
  done
}
...
case "$1" in
  ...
  *)
    if [[ "$1" == "--resume" && -n "$2" ]]; then
      ...
      # No id: pick a session from this project's claude transcripts (alias,
      # model shown; Del deletes) instead of dropping into claude's raw picker.
      _ca_cmd_resume || { _ca_restore_opts; return 0 2>/dev/null || exit 0; }
      _ca_resume_session "$_ca_pick_id"
      set -- --resume "$_ca_pick_id"
